...
export function TripsPage() {
  ...
  return <section>
    <PageHeader eyebrow="My travel life" title="我的旅行" description="查看你的旅行足迹、年度统计和所有旅程。" action={<button type="button" aria-label="打开设置" onClick={() => updateParams({ settings: true })} className="grid size-11 place-items-center rounded-xl border border-line bg-white text-muted hover:text-ink"><Settings className="size-4" /></button>} />
    ...
    <section aria-label="旅行时间轴" className="mt-8"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold text-brand">TRIP TIMELINE</p><h2 className="mt-1 text-2xl font-bold">{selectedYear ?? '全部'}旅行时间轴</h2></div>{trips.length ? <button type="button" onClick={openNewTrip} className="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-xl bg-ink px-4 text-sm font-semibold text-white"><Plus className="size-4" />新建旅行</button> : null}</div>
