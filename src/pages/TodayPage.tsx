...
export function TodayPage(){
 const {selectedTrip,addItineraryItem,updateItineraryStatus,isSaving}=useTrips();const archive=useArchive();const {isOnline}=useNetwork();const [form,setForm]=useState<'itinerary'|'expense'|null>(null);
 ...
 return <section>
  ...
  <div className="mt-7 rounded-3xl border border-line bg-white p-5 shadow-card sm:p-7"><div className="flex items-center justify-between"><div><p className="text-xs font-bold text-brand">TODAY PLAN</p><h2 className="mt-1 text-xl font-bold">今天全部行程</h2></div><span className="text-xs text-muted">{items.length} 项</span></div>{items.filter(x=>x.time!==null).length?<ol className="mt-4 divide-y divide-line">{items.filter(x=>x.time!==null).sort((a,b)=>a.time!.localeCompare(b.time!)).map(item=><TodayItem key={item.id} item={item} busy={!isOnline||isSaving} update={updateItineraryStatus}/>)}</ol>:<p className="py-10 text-center text-sm text-muted">今天还没有定时行程。</p>}
  {state.untimed.length?<><h3 className="mt-6 border-t border-line pt-5 text-sm font-bold">其他安排</h3><ol className="divide-y divide-line">{state.untimed.map(item=><TodayItem key={item.id} item={item} busy={!isOnline||isSaving} update={updateItineraryStatus}/>)}</ol></>:null}</div>
 </section>;
}
