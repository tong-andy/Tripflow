...
export function ArchiveProvider({ children, repository = supabaseArchiveRepository }: { children: ReactNode; repository?: ArchiveRepository }) {
  ...
  const [loadedTripId,setLoadedTripId]=useState('');
  const saving=useRef(false); const request=useRef(0);
  const load=useCallback(async()=>{ const userId=user?.id; const tripId=selectedTripId; const id=++request.current; if(!userId||!tripId){setData(empty);setLoadedTripId('');setLoading(false);return;} setLoading(true);setError(null);try{const next=await repository.loadArchive(userId,tripId);if(id===request.current){setData(next);setLoadedTripId(tripId);}}catch(e){if(id===request.current){setLoadedTripId('');setError(e instanceof Error?e.message:'记录数据加载失败。');}}finally{if(id===request.current)setLoading(false);}},[repository,selectedTripId,user?.id]);
  useEffect(()=>{queueMicrotask(()=>void load());},[load]);
  async function mutate<T>(action:()=>Promise<T>, commit:(value:T)=>void){if(saving.current)throw new Error('正在保存上一项操作，请稍候。');saving.current=true;setSaving(true);setError(null);try{const value=await action();commit(value);}catch(e){setError(e instanceof Error?e.message:'记录数据操作失败。');throw e;}finally{saving.current=false;setSaving(false);}}
  ...
  const visibleData=loadedTripId===selectedTripId?data:empty;
  return <ArchiveContext.Provider value={{...visibleData,isLoading,isSaving,error,retry:load,clearError:()=>setError(null),saveExpense,deleteExpense,savePurchase,deletePurchase,saveMediaNote,deleteMediaNote,saveJournal,deleteJournal}}>{children}</ArchiveContext.Provider>;
}
