...
function fail(error: { message: string } | null) {
  ...
  if (/duplicate|unique/i.test(error.message)) throw new Error('这一天已经有一条回忆。');
  throw new Error('记录数据操作失败，请稍后重试。');
}
...
export function createSupabaseArchiveRepository(client?: SupabaseClient<Database>): ArchiveRepository {
  ...
  return {
    ...
    async createJournal(userId,tripId,input) { requireUser(userId); const row=await one<JournalRow>(db().from('journals').insert({user_id:userId,trip_id:tripId,trip_day_id:input.tripDayId,content:input.content.trim(),rating:input.rating}).select('*').single(),'云端未返回回忆。'); return mapJournal(row); },
    async updateJournal(userId,id,input) { requireUser(userId); const row=await one<JournalRow>(db().from('journals').update({content:input.content.trim(),rating:input.rating}).eq('id',id).eq('user_id',userId).select('*').single(),'找不到回忆。'); return mapJournal(row); },
