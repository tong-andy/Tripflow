...
export function ExpenseForm({trip,item,busy,onCancel,onSubmit}:{trip:Trip;item?:Expense;busy:boolean;onCancel():void;onSubmit(v:CreateExpenseInput):Promise<void>}){
 ...
 const [amountInput,setAmountInput]=useState(item ? String(item.amount) : '');
 async function submitForm(e:FormEvent){e.preventDefault();const amount=Number(amountInput);if(busy||!v.title.trim()||amountInput.trim()===''||!Number.isFinite(amount)||amount<0)return;await onSubmit({...v,amount,currency:v.currency.toUpperCase()});}
 return <form onSubmit={e=>void submitForm(e).catch(()=>undefined)} className="mt-6 grid gap-4 rounded-2xl border border-brand/20 bg-brand-soft/50 p-4 sm:grid-cols-2 lg:grid-cols-3">
  ...
  <label><span className="field-label">金额</span><input aria-label="消费金额" required min="0" step="0.01" inputMode="decimal" type="number" value={amountInput} onChange={e=>setAmountInput(e.target.value)} className="field-input" placeholder="0.00"/></label>
...
export function PurchaseForm({trip,item,busy,onCancel,onSubmit}:{trip:Trip;item?:Purchase;busy:boolean;onCancel():void;onSubmit(v:CreatePurchaseInput):Promise<void>}){
 const [v,setV]=useState<CreatePurchaseInput>({date:item?.date??trip.startDate,title:item?.title??'',amount:item?.amount??0,currency:item?.currency??trip.budgetCurrency??'CNY',location:item?.location??'',recipient:item?.recipient??'',notes:item?.notes??'',organized:item?.organized??false,purchased:item?.purchased??false,includeInExpenses:item?.includeInExpenses??true});
 const [amountInput,setAmountInput]=useState(item ? String(item.amount) : '');
 return <form onSubmit={e=>{e.preventDefault();const amount=Number(amountInput);if(!busy&&amountInput.trim()!==''&&Number.isFinite(amount)&&amount>=0)void onSubmit({...v,amount,currency:v.currency.toUpperCase()}).catch(()=>undefined)}} className="mt-6 grid gap-4 rounded-2xl border border-brand/20 bg-brand-soft/50 p-4 sm:grid-cols-2 lg:grid-cols-3">
 {([['购物日期','date','date'],['物品名称','title','text'],['币种','currency','text'],['地点','location','text'],['收礼人','recipient','text']] as const).map(([label,key,type])=><label key={key}><span className="field-label">{label}</span><input aria-label={label} required={['date','title','currency'].includes(key)} maxLength={key==='currency'?3:undefined} type={type} value={v[key]} onChange={e=>setV({...v,[key]:key==='currency'?e.target.value.toUpperCase():e.target.value})} className="field-input"/></label>)}
 <label><span className="field-label">金额</span><input aria-label="金额" required min="0" step="0.01" inputMode="decimal" type="number" value={amountInput} onChange={e=>setAmountInput(e.target.value)} className="field-input" placeholder="0.00"/></label>
 <div className="flex flex-col gap-3 rounded-xl border border-line bg-white p-4 sm:col-span-2 lg:col-span-3 sm:flex-row"><label className="flex items-center gap-2 text-sm font-semibold"><input aria-label="已购买" type="checkbox" checked={v.purchased} onChange={e=>setV({...v,purchased:e.target.checked})} className="size-4"/>已购买</label><label className="flex items-center gap-2 text-sm font-semibold"><input aria-label="计入花费" type="checkbox" disabled={!v.purchased} checked={v.includeInExpenses} onChange={e=>setV({...v,includeInExpenses:e.target.checked})} className="size-4"/>计入花费</label><span className="text-xs text-muted sm:ml-auto">未购买商品不会计入总支出</span></div>
...
export function JournalForm({trip,item,dayId,busy,onCancel,onSubmit}:{trip:Trip;item?:Journal;dayId?:string;busy:boolean;onCancel():void;onSubmit(v:CreateJournalInput):Promise<void>}){
 ...
 <label><span className="field-label">旅行日期</span><select aria-label="回忆日期" disabled={!!item} value={v.tripDayId} onChange={e=>setV({...v,tripDayId:e.target.value})} className="field-input">{trip.days.map(d=><option key={d.id} value={d.id}>Day {d.dayNumber} · {d.date}</option>)}</select></label>
 <label><span className="field-label">评分（可选）</span><select aria-label="回忆评分" value={v.rating??''} onChange={e=>setV({...v,rating:e.target.value?Number(e.target.value):null})} className="field-input"><option value="">不评分</option>{[1,2,3,4,5].map(x=><option key={x} value={x}>{x} 星</option>)}</select></label>
 <label className="sm:col-span-2"><span className="field-label">回忆内容</span><textarea aria-label="回忆内容" required value={v.content} onChange={e=>setV({...v,content:e.target.value})} className="field-input min-h-40" placeholder="记录今天发生的事……"/></label><div className="sm:col-span-2"><FormActions busy={busy} editing={!!item} onCancel={onCancel}/></div>
 </form>;
}
