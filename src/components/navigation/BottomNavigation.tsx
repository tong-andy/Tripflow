...
export function BottomNavigation() {
  ...
  const active = selectedTrip && getTripStatus(selectedTrip) === 'active';
  const items = active
    ? [{ label: '今天', shortLabel: '今天', to: '/today', icon: CalendarCheck2 }, ...navigationItems.slice(1)]
    : navigationItems;
  return (
    <nav
      aria-label="移动端主导航"
    ...
    >
      <div className="mx-auto grid max-w-lg grid-cols-5">
        {items.map(({ shortLabel, to, icon: Icon }) => (
          <NavLink
            key={to}
            to={to}
            className={({ isActive }) =>
