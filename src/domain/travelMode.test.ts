import { externalNavigationUrl, getTodayItineraryState, getTodayTripDay, getTripStatus, getZonedClock } from './travelMode';
...
describe('external map navigation',()=>{
 it('builds provider-specific web links and resolves system maps by platform',()=>{expect(externalNavigationUrl('上海 外滩','amap')).toContain('uri.amap.com');expect(externalNavigationUrl('上海 外滩','baidu')).toContain('api.map.baidu.com');expect(externalNavigationUrl('上海 外滩','google')).toContain('google.com/maps');expect(externalNavigationUrl('上海 外滩','system','iPhone')).toContain('maps.apple.com');expect(externalNavigationUrl('上海 外滩','system','Android')).toContain('google.com/maps');});
});
